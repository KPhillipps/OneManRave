...
void initializeVirtualStrips() {
    ...
    for (int physStrip = 0; physStrip < NUM_APA102_STRIPS; physStrip++) {
        ...
        // First half (LEDs 0-143) - already going UP, map directly
        for (int i = 0; i < LEDS_PER_VIRTUAL_STRIP; i++) {
            virtualLeds[virtualBase][i] = &leds[baseIndex + i];
        }

        // Second half (LEDs 144-287) - going DOWN, reverse to appear UP
        for (int i = 0; i < LEDS_PER_VIRTUAL_STRIP; i++) {
            virtualLeds[virtualBase + 1][i] = &leds[baseIndex + LEDS_PER_PHYSICAL_STRIP - 1 - i];
        }
    }

    Serial.println("Virtual strips initialized (all going UP, left to right).");
}
...
void virtualStripTest() {
    ...
    fill_solid(leds, 1728, CRGB::Black);
    FastLED.show();
    Serial.println("Virtual strip test complete.");
}
...
void setup() {
    ...
    FastLED.addLeds<APA102, 6,  CLOCK_PIN, BGR>(leds, 288).setCorrection(TypicalLEDStrip);
    FastLED.addLeds<APA102, 7,  CLOCK_PIN, BGR>(leds + 288, 288).setCorrection(TypicalLEDStrip);
    FastLED.addLeds<APA102, 2,  CLOCK_PIN, BGR>(leds + 576, 288).setCorrection(TypicalLEDStrip);
    FastLED.addLeds<APA102, 21,  CLOCK_PIN, BGR>(leds + 864, 288).setCorrection(TypicalLEDStrip);
    FastLED.addLeds<APA102, 5, CLOCK_PIN, BGR>(leds + 1152, 288).setCorrection(TypicalLEDStrip);
    FastLED.addLeds<APA102, 8,  CLOCK_PIN, BGR>(leds + 1440, 288).setCorrection(TypicalLEDStrip);
